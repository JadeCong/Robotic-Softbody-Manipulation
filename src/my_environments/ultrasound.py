...
class Ultrasound(SingleArmEnv):
    ...
    def __init__(
        ...
        # desired states
        self.goal_quat = np.array([-0.69192486, 0.72186726, -0.00514253,
                                   -0.01100909])  # Upright probe orientation found from experimenting (x,y,z,w)
        ...
        self.scaning_full_traj_once = False
    ...
    def reward(self, action=None):
        ...
        self.force_reward = self.force_reward_mul * np.exp(
            -1 * self.force_error) if self._check_probe_contact_with_torso() else 0
        ...
        self.der_force_reward = self.der_force_reward_mul * np.exp(
            -1 * self.der_force_error) if self._check_probe_contact_with_torso() else 0
    ...
    def _reset_internal(self):
        ...
        if not self.deterministic_reset:
            ...
            for obj_pos, _, obj in object_placements.values():
                self.sim.data.set_joint_qpos(obj.joints[0],
                                             np.concatenate([np.array(obj_pos), np.array([0.5, 0.5, -0.5, -0.5])]))
        ...
        self.torso_mesh_body_pos_world_array, _ = self.torso.get_body_world_pose_array(self._torso_xpos,
        ...
        self.torso_mesh_body_quat_world_array = self.torso.fit_body_world_quat_array(self.torso_start_index,
                                                                                     self.torso_up_surface_index,
                                                                                     self.torso_mesh_body_pos_world_array)
        ...
        self.traj_ori = T.quat2axisangle(
            T.mat2quat(np.array(self.trajectory.eval(self.traj_step)[0]).reshape(3, 3)))  # (by JadeCong)
    ...
    def _post_action(self, action):
        ...
        self._add_waypoint_site(self.viewer.viewer, "World_Origin", origin_size=[0.01, 0.01, 0.01],
                                axis_size=[0.005, 0.005, 0.8],
        ...
        for idx in iter(range(len(self.torso_up_surface_index))):
            self._add_waypoint_site(self.viewer.viewer, str(idx), origin_size=[0.001, 0.001, 0.001],
                                    axis_size=[0.0005, 0.0005, 0.05],
                                    pos=self.torso_scanning_order_traj['wp_{}'.format(idx)]['pos'],
                                    quat=self.torso_scanning_order_traj['wp_{}'.format(idx)]['ori'])
            if idx < (len(self.torso_up_surface_index) - 1):
                self._add_waypoint_line(self.viewer.viewer, "", rgba=[0, 1, 0.7, 1],
                                        pos_start=self.torso_scanning_order_traj['wp_{}'.format(idx)]['pos'],
                                        pos_end=self.torso_scanning_order_traj['wp_{}'.format(idx + 1)]['pos'])
        ...
        print("traj_step: {}%".format(
            self.traj_step / (self.num_waypoints - 1) * 100))  # print the accomplish percent every step(by JadeCong)
        ...
        if self.save_data:
            ...
            self.data_ee_diff_quat[self.timestep - 1] = distance_quat(convert_quat(self._eef_xquat, to="wxyz"),
                                                                      convert_quat(T.axisangle2quat(self.traj_ori),
                                                                                   to="wxyz"))  # (by JadeCong)
            ...
            self.data_time[self.timestep - 1] = (
                                                            self.timestep - 1) / self.horizon * 100  # percentage of completed episode
        ...
        if done and self.save_data:
            ...
            self._save_data(self.data_ee_z_goal_derivative_contact_force, sim_data_fldr,
    ...
    def get_sacanning_trajectory(self):  # (by JadeCong)
        ...
        """
        self.torso_scanning_waypoint_array = self.get_torso_scanning_waypoint_array(self.torso_up_surface_index,
                                                                                    self.torso_mesh_body_pos_world_array,
                                                                                    self.torso_mesh_body_quat_world_array,
                                                                                    self.goal_velocity,
                                                                                    self.goal_contact_z_force)  # get the torso waypoint array(by JadeCong)
    ...
    def get_torso_scanning_delta_trajectory(self, wp_num):  # (by JadeCong)
        ...
        if self.scan_beginning:
            for idx in iter(range(wp_num)):
                ...
                wp_scanning.append(
                    np.append(quat2mat(self.torso_scanning_order_traj['wp_{}'.format(idx)]['ori']).flatten(),
            ...
            if self.scanning_index <= len(self.torso_up_surface_index) - wp_num:
                for idx in iter(range(wp_num)):
                    ...
                    wp_scanning.append(np.append(quat2mat(
                        self.torso_scanning_order_traj['wp_{}'.format(idx + self.scanning_index)]['ori']).flatten(),
                                                 self.torso_scanning_order_traj[
                                                     'wp_{}'.format(idx + self.scanning_index)][
                                                     'pos']))  # for wp_num >= 2

            elif (self.scanning_index > len(self.torso_up_surface_index) - wp_num) and (
                    self.scanning_index < len(self.torso_up_surface_index) - 1):
    ...
    def get_torso_scanning_order_trajectory(self):  # (by JadeCong)
        ...
        for idx_y in iter(range(torso_scanning_row)):
            for idx_x in iter(range(torso_scanning_column)):
                if idx_y & 1 == 0:
                    torso_scanning_order_traj['wp_{}'.format(idx_y * torso_scanning_column + idx_x)] = \
                    torso_scanning_wp_array[idx_y][idx_x]
                else:
                    torso_scanning_order_traj['wp_{}'.format(idx_y * torso_scanning_column + idx_x)] = \
                    torso_scanning_wp_array[idx_y][torso_scanning_column - 1 - idx_x]

        return torso_scanning_order_traj

    def get_torso_scanning_waypoint_array(self, torso_scanning_index_list, torso_body_pos_world_array,
                                          torso_body_quat_world_array, goal_vel, goal_force):  # (by JadeCong)
        """
        ...
        torso_scanning_wp_array = [[0 for i in range(torso_scanning_column)] for j in
                                   range(len(self.torso_start_index))]  # j for row and i for column
        ...
        for idx_y in iter(range(len(self.torso_start_index))):
            for idx_x in iter(range(torso_scanning_column)):
                ...
                # ori = T.mat2quat(np.array(self.random_quat_sample(np.array([-0.69192486, 0.72186726, -0.00514253, -0.01100909]), 0.00001)).reshape(3, 3))  # xyzw
                # ori = convert_quat(torso_body_quat_world_array[torso_scanning_index_list[idx_y * torso_scanning_column + idx_x]], to="xyzw")  # xyzw
                # ori = T.quat_multiply(np.array([0, -0.7071068, 0, 0.7071068]), ori)  # rotate the ori
    ...
    def random_quat_sample(self, ref_quat, var):
        ...
        import random
        q = [random.gauss(ref_quat[0], var), random.gauss(ref_quat[1], var), random.gauss(ref_quat[2], var),
             random.gauss(ref_quat[3], var)]
        q = vectorops.unit(q)
    ...
    def _get_sacanning_trajectory(self):  # (by JadeCong)
        ...
        Returns:
            (klampt.model.trajectory Object):  trajectory
        """
        self.torso_scanning_waypoint_array = self._get_torso_scanning_waypoint_array(self.grid_pts_x, self.grid_pts_y,
                                                                                     self.goal_quat, self.goal_velocity,
                                                                                     self.goal_contact_z_force)  # get the torso waypoint array(by JadeCong)
    ...
    def _get_torso_scanning_delta_trajectory(self, wp_num):  # (by JadeCong)
        ...
        if self.scan_beginning:
            ...
            if self.scanning_index <= self.grid_pts_x * self.grid_pts_y - wp_num:
                for idx in iter(range(wp_num)):
                    wp_scanning.append(self.torso_scanning_order_traj['wp_{}'.format(idx + self.scanning_index)][
                                           'pos'])  # for wp_num >= 2
    ...
    def _get_torso_scanning_order_trajectory(self):  # (by JadeCong)
        ...
        for idx_y in iter(range(self.grid_pts_y)):
            for idx_x in iter(range(self.grid_pts_x)):
                if idx_y & 1 == 0:
                    torso_scanning_order_traj['wp_{}'.format(idx_y * self.grid_pts_x + idx_x)] = \
                    torso_scanning_wp_array[idx_y][idx_x]
                else:
                    torso_scanning_order_traj['wp_{}'.format(idx_y * self.grid_pts_x + idx_x)] = \
                    torso_scanning_wp_array[idx_y][self.grid_pts_x - 1 - idx_x]

        return torso_scanning_order_traj

    def _get_torso_scanning_waypoint_array(self, x_axis_points, y_axis_points, goal_ori, goal_vel,
                                           goal_force):  # (by JadeCong)
        """
        Creates a torso waypoint array as the keypoints of scanning trajectory according to given points in x and y axis.
        A waypoint of trajectory contains position, orientation, velocity(linear and angular) and contact force(option).
        ...
        # Initialize the dict for waypoint of scanning trajectory
        torso_scanning_wp_array = [[0 for i in range(x_axis_points)] for j in
                                   range(y_axis_points)]  # j for row and i for column
    ...
    def _add_waypoint_site(self, viewer, name="wp", origin_size=[0.006, 0.006, 0.006], axis_size=[0.002, 0.002, 0.2],
        ...
        viewer.add_marker(type=100,  # type=const.GEOM_ARROW,  # 5 for cylinder and 100 for arrow
                          size=axis_size,
                          pos=np.array(pos),
                          mat=(R.from_quat(quat[[0, 1, 2, 3]]) * R.from_euler('xyz', [0, 90, 0],
                                                                              degrees=True)).as_matrix().flatten(),
                          # transfer the index from wxyz to xyzw
                          rgba=[1, 0, 0, 1],
                          label="")
        viewer.add_marker(type=100,  # type=const.GEOM_ARROW,  # 5 for cylinder and 100 for arrow
                          size=axis_size,
                          pos=np.array(pos),
                          mat=(R.from_quat(quat[[0, 1, 2, 3]]) * R.from_euler('xyz', [-90, 0, 0],
                                                                              degrees=True)).as_matrix().flatten(),
                          # transfer the index from wxyz to xyzw
                          rgba=[0, 1, 0, 1],
                          label="")
        viewer.add_marker(type=100,  # type=const.GEOM_ARROW,  # 5 for cylinder and 100 for arrow
                          size=axis_size,
                          pos=np.array(pos),
                          mat=(R.from_quat(quat[[0, 1, 2, 3]]) * R.from_euler('xyz', [0, 0, 0],
                                                                              degrees=True)).as_matrix().flatten(),
                          # transfer the index from wxyz to xyzw
                          rgba=[0, 0, 1, 1],
                          label="")
    ...
    def _add_waypoint_line(self, viewer, name="line", rgba=[0, 1, 0.5, 1], pos_start=[0, 0, 0], pos_end=[1, 1, 1]):
        ...
        viewer.add_marker(type=103,  # type=const.GEOM_LINE,  # 103 for line
                          ...
                          mat=R.from_rotvec(
                              get_rotation_vector(np.array([0, 0, 1]), (pos_end - pos_start))).as_matrix().flatten(),
                          # transfer the index from wxyz to xyzw
                          rgba=rgba,
                          label=name)
    ...
    def _get_initial_qpos(self):
        ...
        # ori_euler = mat2euler(T.quat2mat(T.axisangle2quat(self.traj_ori)))  # (by JadeCong)
